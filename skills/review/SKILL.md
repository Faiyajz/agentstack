...
Inspect the diff, surrounding code, tests, and public contracts. Check correctness, error paths, compatibility, security, data safety, concurrency, performance cliffs, accessibility for UI, and operational risk where relevant.
...
If there are no findings, say so clearly. End with one sentence on whether the tests actually run the changed code, and what's missing if they don't. Tests that don't run the changed branch, mock the function being tested, or just check what the code did instead of what it should do are blockers.
