...
If you're not sure, ask. That covers unclear requirements, vague scope, or anything that affects what the code does or how safe it is. If the task is too large, ask for it to be split. If the task, spec, or plan is wrong, stop and update the instruction source instead of coding around it.
...
Edit only the files the task needs. For multi-part tasks, work in small steps that each run. Handle the important ways the code can fail. Preserve user changes already in the worktree.

### 4. Test and verify

Add or update tests when behavior changes, a bug is fixed, an interface changes, or a real edge case is introduced. Run the tests for the change, then the project's wider checks — including the full test suite if you can. Use browser or smoke verification for UI, deployment, or runtime behavior. Fix what the checks catch without going beyond the task.
...
- Do not claim done until the changed path has been exercised or you have reported why it could not be.
