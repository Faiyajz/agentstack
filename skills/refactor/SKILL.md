...
- Keep compatibility shims only when callers still need them; otherwise delete dead paths with proof.

### 4. Verify

- Run the relevant tests and checks after editing.
- Run broader tests when the refactor touches shared behavior or contracts.
- Compare behavior before and after when tests are weak or missing.
