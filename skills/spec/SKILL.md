---
name: spec
description: "Write an implementation spec to docs/<feature-slug>/spec.md and pause for human review. Use when the user says \"write a spec\", \"spec this out\", \"technical design\", or when a task has decisions, invariants, or contracts that should be reviewed before code is written. Use design-doc instead for broad architecture tradeoffs."
user-invocable: true
...
- Identify migration, rollout, rollback, observability, security, privacy, and compatibility concerns when they affect implementation.
...
- **Rollout and Backout** *(when relevant)*: migration, deployment, smoke checks, and rollback.
...
- Keep domain implementation skills such as `backend-api`, `frontend-ui`, or `devops-deploy` in the implementation phase; they do not replace reviewable specs for contract-changing work.
